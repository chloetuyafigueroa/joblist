package rest.omms;


import java.sql.SQLException;
import java.text.DateFormat;
import java.text.ParseException;
import java.text.SimpleDateFormat;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.Calendar;
import java.util.Date;
import java.util.Iterator;
import java.util.List;
import java.util.Scanner;
import java.util.concurrent.TimeUnit;

import javax.servlet.ServletException;

//import org.apache.log4j.PropertyConfigurator;
import org.json.JSONArray;
import org.json.JSONException;
import org.json.JSONObject;

import jssc.SerialPort;
import jssc.SerialPortException;
import jssc.SerialPortEvent;
import jssc.SerialPortEventListener;
import rest.omms.PDUConverter.PDUEncoded;
import rest.omms.gsm3040.Pdu;
import rest.omms.gsm3040.PduParser;
import rest.omms.gsm3040.PduUtils;

public class SmsModule {
	private SerialPort serialPort;
	private volatile boolean listening = true; // Flag to control listening
	 public SmsModule(String portName) {
	        serialPort = new SerialPort(portName);
	        MetaData.serialPort=serialPort;
	        
	    }
	 private SerialPortEventListener eventListener;

	  String sp="COM4";
	  char ch = '"';
	  String ctz= Character.toString ((char) 26)+ "\r";//
	//public  SerialPort serialPort;
	public  void main(String args[]) throws IllegalAccessException, SerialPortException, InterruptedException {
		
	    	//NotificationServer.sendNotification("Your notification message here");
	    
	
	}
	public void restart() throws ServletException {
		System.out.println("SmsModule Stop!");
       
    }
	public  String ArrToString(JSONArray jArr) {
		String x="";
		for(int i=0;i<jArr.length();i++) {
			x=x+jArr.get(i);
		}
		return x;
	}
	public  Boolean initializeLogger=false;
	
	public  Boolean isOpened=false;									
	
	public  void connect() throws SerialPortException {
		if(!MetaData.isOpened) {
	    MetaData.serialPort.openPort();
	    MetaData.serialPort.setParams(SerialPort.BAUDRATE_9600, SerialPort.DATABITS_8,
                              SerialPort.STOPBITS_1, SerialPort.PARITY_NONE);
	    
	    configureForIncomingMessages();
        /**
	    MetaData.serialPort.addEventListener(new SerialPortEventListener() {
            @Override
            public void serialEvent(SerialPortEvent event) {
                if (event.isRXCHAR()) {
                    try {
                    	String receivedData = serialPort.readString();
                        if (receivedData != null) {
                        	//handleIncomingMessage(receivedData);
                        }
                    } catch (SerialPortException ex) {
                        ex.printStackTrace();
                    }
                }
            }
        });/**/
        MetaData.isOpened=true;
		}
    }
	public void startSMSListening() throws SerialPortException {
		System.out.println("...startSMSListening");
		//configureForIncomingMessages();
	    	eventListener = new SerialPortEventListener() {
	            @Override
	            public void serialEvent(SerialPortEvent event) {
	                if (event.isRXCHAR() && event.getEventValue() > 0) {
	                    try {
	                    	
	                        String receivedData = MetaData.serialPort.readString(event.getEventValue());
	                        System.out.println("All- SMSListening:"+receivedData);
	                        if (receivedData != null && receivedData.contains("+CMT:")) {
	                        System.out.println("Incoming-SMSListening:"+receivedData);
	                        }
	                        //processReceivedSms(receivedData);
	                    } catch (SerialPortException ex) {
	                        ex.printStackTrace();
	                    }
	                  }
	              }
	            };
	            try {
	            	MetaData.serialPort.addEventListener(eventListener, SerialPort.MASK_RXCHAR);
	            } catch (SerialPortException ex) {
	                ex.printStackTrace();
	            }
	    
	}
	public void stopListening() throws SerialPortException {
		if(MetaData.isOpened) {
	    	listening = false;
	    	 //MetaData.isOpened=false;
	    	 /**MetaData.serialPort.closePort();
	    	 try {
	         	MetaData.serialPort.removeEventListener();
	         } catch (SerialPortException ex) {
	             ex.printStackTrace();
	         }/**/
	    }
        
    }
    public void disconnect() throws SerialPortException {
    	if(MetaData.isOpened) {
    		MetaData.serialPort.writeString("ATZ\r");
    	listening = false;
    	 MetaData.isOpened=false;
    	 MetaData.serialPort.closePort();
    	}
    }
    private  void configureForIncomingMessages() throws SerialPortException {
    	 MetaData.serialPort.writeString("AT+CMGF=1\r"); // Set PDU mode
        delay(1000);

        MetaData.serialPort.writeString("AT+CNMI=2,1,0,0,0" + "\r");; // Notify on new messages
        delay(1000);
    }
    public void debugConnection() {
        try {
        	
            // Check basic communication
        	MetaData.serialPort.writeString("AT\r");
            String response = MetaData.serialPort.readString();
            System.out.println("AT Command Response: " + response);
            delay(1000);
            // Check SMS mode
            MetaData.serialPort.writeString("AT+CMGF?\r");
            response = MetaData.serialPort.readString();
            System.out.println("Current SMS Mode: " + response);
            delay(1000);
            // Check message notification settings
            MetaData.serialPort.writeString("AT+CNMI?\r");
            response = serialPort.readString();
            System.out.println("Message Notification Settings: " + response);

        } catch (SerialPortException e) {
            e.printStackTrace();
        }
    }

    private  void delay(int milliseconds) {
        try {
            Thread.sleep(milliseconds);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
	public  void startListening() throws SerialPortException {
		if(MetaData.isOpened) {
		configureForIncomingMessages();
		System.out.println("Start listening...");
		Thread listenerThread = new Thread(() -> {
			while (listening) {
				System.out.println("...");
                try {
                    synchronized (serialPort) {
                        String response =  serialPort.readString();
                        if (response != null) {                        	
                        	System.out.println("Incoming response:"+response);
                        	//handleIncomingMessage(response);
                            
                        }
                        if (response != null && response.contains("+CMT:")) {                        	
                        	System.out.println("Incoming SMS response:"+response);
                        	//handleIncomingMessage(response);
                            
                        }
                    }
                    TimeUnit.MILLISECONDS.sleep(1000);	
                } catch (SerialPortException | InterruptedException e) {
                    e.printStackTrace();
                }
            }
        });
        listenerThread.setDaemon(true);
        listenerThread.start();
		}
    }

    private  void handleIncomingMessage(String response) {
        // Example of parsing PDU response
    	JSONObject jsonObject=new JSONObject();
    	String pduString = response.substring(response.indexOf("+CMT:") + 5);
    	System.out.println("PDU String:"+pduString);
    	try {
			 Pdu pdu = new PduParser().parsePdu(pduString);
		
       	  long millis = ((Calendar)pdu.getSmscTimeStamp()).getTimeInMillis();
         
       	  jsonObject.put("sender",pdu.getAddress());
       	  //jsonObject.put("rawpdu",pdu.getRawPdu());
       	  jsonObject.put("millis",new JSONArray("[\""+String.valueOf(millis)+"\"]"));
       	  jsonObject.put("isConcat",pdu.isConcatMessage());
       	  jsonObject.put("MpMaxNo",1);
       	  jsonObject.put("MpSeqNo",new JSONArray("[\"1\"]"));
       	 if(pdu.isConcatMessage()) {
	        	  jsonObject.put("UDH",toHexString(pdu.getUDHData(), 4));
	        	  jsonObject.put("MpMaxNo",pdu.getMpMaxNo());
	        	  jsonObject.put("MpSeqNo",new JSONArray("[\""+pdu.getMpSeqNo()+"\"]"));
       	 }
       	 JSONArray jData=new JSONArray();
       	 jsonObject.put("data",jData.put(pdu.getDecodedText()));
       	 UpsertSMS(jsonObject);
       	 try {
				UserService.storeSMS(jsonArray,null);
			} catch (IllegalAccessException | SQLException | InterruptedException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			//e.printStackTrace();
			//deleteSMS(jsonObject,sp);
		}
        
        
        System.out.println("Received SMS: " + jsonObject);
    }
	public  String deleteSMS(int x, int y, String sp) {
		 System.out.println("From deleteSMS SP:"+sp);/**/
		 synchronized (MetaData.serialPort) {
			    try {		        	
			        serialPort.writeString("AT+CMGD="+x+","+y+"\r");
			        TimeUnit.MILLISECONDS.sleep(1000);	
			        smsPdu=MetaData.serialPort.readString();
			        System.out.println("From deleteSMS SerialPort.readString():");
			        System.out.println("deleting:"+smsPdu);
		        
		        } catch (jssc.SerialPortException e) {
		        System.out.println("From deleteSMS:"+e);/**/
		       // log.debug("From deleteSMS:"+e);
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
					//log.debug("From deleteSMS:"+e);
				}
		 }
	        
	     
		 return smsPdu;
	}
	public  void deleteSMS(JSONArray cMList,String sp) {
		   System.out.println("From UserService.deleteSMS()");
		   for(int i=0;i<cMList.length();i++) {
			   System.out.println(cMList.get(i));
			   String cMGL=cMList.get(i).toString();
			   Boolean deleted=false;
			   while(!deleted) {
		            deleted=deleted(MetaData.smsModule.deleteSMS(Integer.valueOf(cMGL), 0,sp));
			   //     SMSTranceiver.deleteSMS(Integer.valueOf(cMGL), 0,sp);
			   }
		   }
	   }
	   public  Boolean deleted(String xString)  {
	   	Boolean deleted=false;
	   	int okCount=0;
	   	System.out.println((xString));
	   	if(xString!=null) {
		    	Scanner scanner = new Scanner(xString);
		    	while (scanner.hasNextLine()) {
		    		String line = scanner.nextLine();
			         if(line.equals("OK")) { 
			        	 okCount++;
			         }   
		        }       
		        scanner.close();
		        if(okCount>0) {deleted=true;}
		        System.out.println("deleted:"+deleted);
	   	}
	   	return deleted;
	   }
	public  void deleteAllSMS(String sp) {
		 System.out.println("From deleteAllSMS SP:"+sp);/**/
		 synchronized (MetaData.serialPort) {	
			    try {		        	
			    	MetaData.serialPort.writeString("AT+CMGL=ALL \r");
			        TimeUnit.MILLISECONDS.sleep(1000);	
			        smsPdu=MetaData.serialPort.readString();
			        System.out.println("deleting:"+smsPdu);
		        
		        } catch (jssc.SerialPortException e) {
		        System.out.println("From deleteSMS:"+e);/**/
		        //log.debug("From deleteSMS:"+e);
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
					//log.debug("From deleteSMS:"+e);
				}			
	     }
	        
	}
	public  String smsReadString="";
	public  JSONArray pDUArray(String xString) {
		//log.info(xString);
		jsonArray=new JSONArray();
		if(xString!=null) {
			Scanner scanner = new Scanner(xString);
	        String pduString="";
	        while (scanner.hasNextLine()) {
	        	/**/String line = scanner.nextLine();
	          
	          JSONObject jsonObject=new JSONObject();
	          if(line.contains("+CMGL:")) { 
	        	  int s=line.indexOf(":");
	             List<String> CMGList=CMGList(line.substring(s+2, line.length()));
	        	  jsonObject.put("CMGL",new JSONArray("[\""+CMGList.get(0)+"\"]"));//
	        	  pduString=scanner.nextLine();
	        	  //System.out.println(pduString.length()/2);
	        	 
	        	  if((pduString.length()/2)==Integer.valueOf(CMGList.get(3))+8) {
	        		  
		        	  
					try {
						 Pdu pdu = new PduParser().parsePdu(pduString);
					
			        	  long millis = ((Calendar)pdu.getSmscTimeStamp()).getTimeInMillis();
			          
			        	  jsonObject.put("sender",pdu.getAddress());
			        	  //jsonObject.put("rawpdu",pdu.getRawPdu());
			        	  jsonObject.put("millis",new JSONArray("[\""+String.valueOf(millis)+"\"]"));
			        	  jsonObject.put("isConcat",pdu.isConcatMessage());
			        	  jsonObject.put("MpMaxNo",1);
			        	  jsonObject.put("MpSeqNo",new JSONArray("[\"1\"]"));
			        	 if(pdu.isConcatMessage()) {
				        	  jsonObject.put("UDH",toHexString(pdu.getUDHData(), 4));
				        	  jsonObject.put("MpMaxNo",pdu.getMpMaxNo());
				        	  jsonObject.put("MpSeqNo",new JSONArray("[\""+pdu.getMpSeqNo()+"\"]"));
			        	 }
			        	 JSONArray jData=new JSONArray();
			        	 jsonObject.put("data",jData.put(pdu.getDecodedText()));
			        	 UpsertSMS(jsonObject);
					} catch (Exception e) {
						// TODO Auto-generated catch block
						//e.printStackTrace();
						//deleteSMS(jsonObject,sp);
					}
		        	 //jsonArray.put(jsonObject);
					//System.out.println(jsonArray);
	        	  }
	          }       
	        }
	        System.out.println(jsonArray);
	        scanner.close();
		}
	     
		return jsonArray;		
	}
	 int cnt=0;
	public  JSONArray readSMSPdu(int TD,String sp) throws IllegalAccessException, SerialPortException {
		JSONArray readSMSPdu=new JSONArray();		
		System.out.println("SmsModule.readSMSPdu(TD)");	  
		if(MetaData.isOpened) {
			synchronized (MetaData.serialPort) {
			    try {  
			    	serialPort.writeString("AT+CMGF=0" + "\r");
			        TimeUnit.MILLISECONDS.sleep(1000);
			        MetaData.serialPort.writeString("AT+CMGL=4" + "\r");	  
			        //serialPort.writeString("AT+CMGR=12" + "\r");	    
			        
			        TimeUnit.MILLISECONDS.sleep(TD);
			        smsReadString="";
			        Boolean completed=false;
			        
			        while(!completed) {
			        	String rS=serialPort.readString();
			        	smsReadString=smsReadString+rS;
			        	completed=completed(rS);
			        	TimeUnit.MILLISECONDS.sleep(1000);
			        	cnt++;
			        	if(cnt>=25) {completed=true;cnt=0;}
			        	System.out.println("smsReadString count:"+cnt);
			        }
			    	System.out.println("smsReadString completed!");
				    
			    	        
			    } catch (jssc.SerialPortException|InterruptedException e) {
			        System.out.println("From readSMSPdu:"+e);
			        //log.debug("From readSMSPdu:"+e);
			    } 
			    try {
					readSMSPdu=pDUArray(smsReadString);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					//e.printStackTrace();
					readSMSPdu=new JSONArray();
				}
			}
		}
	    return readSMSPdu;//readSMSPdu;
	}
	
	public  String readSMS(int TD, String sp) {
		if(MetaData.isOpened) {
		synchronized (serialPort) {
			try {
				System.out.println("SmsModule.readSMS(TD):"+TD+":"+sp);
				MetaData.serialPort.writeString("AT+CMGF=1" + "\r");
		        TimeUnit.MILLISECONDS.sleep(1000);
		        MetaData.serialPort.writeString("AT+CMGL=" +ch+"ALL"+ch+ "\r");
		        TimeUnit.MILLISECONDS.sleep(TD);
		        smsReadString="";
		        Boolean completed=false;
		        while(!completed) {
		        	String rS=MetaData.serialPort.readString();
		        	System.out.println("rS: "+rS);
		        	smsReadString=smsReadString+rS;
		        	completed=completed(rS);
		        	
		        	TimeUnit.MILLISECONDS.sleep(1000);
		        }
		       System.out.println("test: "+smsReadString);
		   /**/ } catch (jssc.SerialPortException|InterruptedException e) {
		        	System.out.println("From readSMS:"+e);/**/
		        	//log.debug("From readSMS:"+e);
		   		}
				
			}
		}
		return smsReadString;
	}
	public  String readSerial(String sp) {
		String xString="";
		synchronized (MetaData.serialPort) {
			try {
				xString=MetaData.serialPort.readString();
			} catch (SerialPortException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
				//log.debug("From readSerial:"+e);
			}
			
		}
		
		return xString;
	}
	public  String CSMS="00";
    public  Boolean sendSMS(String address, String data, String sp) throws SerialPortException   {
    	System.out.println("From sendSMS data:"+data);    	
    	data=data.replace("\\\\", "\\");
    	float i=(float)data.length()/145;
        int s=0;
        int t=145;
        int p=0;
        synchronized (MetaData.serialPort) {
	    	try {
	    		
	    		MetaData.serialPort.writeString("AT+CMGF=0" + "\r");
		        TimeUnit.MILLISECONDS.sleep(1000);
		   	    
		        CSMS=CSMSconverter();
		        //CSMS="00";
		        if(t>data.length()) {
	        		t=data.length();
	        	}
		        do {
		        	try {
		        		
		        		PDUEncoded pduEnc=PDUConverter.encodeSend(address, data.substring(s, t),(int) Math.ceil(i),p+1,CSMS);
		        		MetaData.serialPort.writeString("AT+CMGS=" + pduEnc.getPduLength() + "\r");
				        TimeUnit.MILLISECONDS.sleep(100); 	      
				        MetaData.serialPort.writeString(pduEnc.getPduEncoded() + ctz);
				        //Pdu pdu=new PduParser().parsePdu(pduEnc.getPduEncoded());//pduEnc.getPduEncoded()"001100098121436587F900000B029B17"
				        //System.out.println(pdu);
				        
						TimeUnit.MILLISECONDS.sleep(5000);/**/
						  
		        	} catch (ParseException e) {
						// TODO Auto-generated catch block
						//e.printStackTrace();
					}
		        	p++;
		        	s=t;
		        	t=t+145;
		        	if(t>data.length()) {
		        		t=data.length();
		        	}
		        	
		         } while((int) Math.ceil(i)>p);
		        sentData=MetaData.serialPort.readString();
		        
			} catch (InterruptedException | SerialPortException e) {
				// TODO Auto-generated catch block
				System.out.println("From sendSMS:"+e);
				//log.debug("From sendSMS:"+e);
			}
    	}
        configureForIncomingMessages();
    	return sent((int)Math.ceil(i), sentData);
    }
    public  void sendSMS2(String address, String data, String sp)   {
    	data=data.replace("\\\\", "\\");
    	System.out.println("From sendSMS2 data:"+data);    	
    	float i=(float)data.length()/145;
        int s=0;
        int t=145;
        int p=0;
      	
	    	try {
	    		
		        CSMS=CSMSconverter();
		        //CSMS="00";Xj
		        if(t>data.length()) {
	        		t=data.length();
	        	}
		        do {
		        	try {
		        		
		        		PDUEncoded pduEnc=PDUConverter.encodeSend(address, data.substring(s, t),(int) Math.ceil(i),p+1,CSMS);
						 //Pdu pdu=new PduParser().parsePdu(pduEnc.getPduEncoded());//pduEnc.getPduEncoded()"001100098121436587F900000B029B17"
				        System.out.println("AT+CMGS=" + pduEnc.getPduLength() + "\r");
				        System.out.println(pduEnc.getPduEncoded() + ctz);
						TimeUnit.MILLISECONDS.sleep(5000);/**/
						  
		        	} catch (ParseException e) {
						// TODO Auto-generated catch block
						//e.printStackTrace();
					}
		        	p++;
		        	s=t;
		        	t=t+145;
		        	if(t>data.length()) {
		        		t=data.length();
		        	}
		        	
		         } while((int) Math.ceil(i)>p);
		       
			} catch (InterruptedException  e) {
				// TODO Auto-generated catch block
				
			}
    	
    	
    }
       public  Boolean sent(int i, String xString)  {
    	Boolean sent=false;
    	System.out.println((xString));
    	if(xString!=null) {
	    	Scanner scanner = new Scanner(xString);
	    	int count=0;
	        while (scanner.hasNextLine()) {
	          String line = scanner.nextLine();
	          if(line.contains("OK")) { 
	        	 count++;
	          }        
	        }
	         if(i==count-1) {sent=true;}
	         else {System.out.println(xString);}
	        scanner.close();
	        System.out.println(sent);
    	}
    	return sent;
    }
    public  int okCount=0;
    public  int readingCount=0;
    public  Boolean completed(String xString)  {
    	Boolean completed=false;
    	System.out.println("xString: "+xString);
    	if(xString!=null) {
	    	Scanner scanner = new Scanner(xString);
	    	while (scanner.hasNextLine()) {
	          String line = scanner.nextLine();
	         if(line.equals("OK")) { 
	        	 okCount++;
	         }   
	        }
	    	readingCount++;
	         scanner.close();
	         if(okCount>1) {completed=true;okCount=0;}
	         if(readingCount<5) {completed=true;readingCount=0;}
	         System.out.println("readingCount"+":"+readingCount);   
	        System.out.println(completed+":"+okCount);
    	}
    	return completed;
    }
   
    public  void sendSMS2() {
    	try {
    		MetaData.serialPort.writeString("AT+CMGF=0" + "\r");
	        TimeUnit.MILLISECONDS.sleep(1000);
	   	   
	        MetaData.serialPort.writeString("AT+CMGS=130"  + "\r");//154
	        TimeUnit.MILLISECONDS.sleep(100); 	      
	        //serialPort.writeString("" + ctz);
	        MetaData.serialPort.writeString(smsPdu + ctz);
			TimeUnit.MILLISECONDS.sleep(5000);
			
			/**serialPort.writeString("AT+CMGS=154"  + "\r");
	        TimeUnit.MILLISECONDS.sleep(100); 	      
	        serialPort.writeString("" + ctz);
			TimeUnit.MILLISECONDS.sleep(5000);
			
			serialPort.writeString("AT+CMGS=143"  + "\r");
	        TimeUnit.MILLISECONDS.sleep(100); 	      
	        serialPort.writeString("" + ctz);
			TimeUnit.MILLISECONDS.sleep(5000);**/
	        
	        	
	        System.out.println(serialPort.readString());			
			//serialPort.closePort();//Close serial port  
	        
	   /**/ } catch (jssc.SerialPortException e) {
	        System.out.println(e);/**/
	    
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
    	
    }
    public  JSONArray jsonArray=new JSONArray();
	public  String PDUtoText(String resultMessage) {
			Pdu pdu = new PduParser().parsePdu(resultMessage);
			System.out.println("PDU:"+pdu);	       
			 byte[] bytes = pdu.getUserDataAsBytes();
	        //System.out.println("Address:"+pdu.getAddress());
	        SimpleDateFormat df = new SimpleDateFormat("dd-MMM-yyyy hh:mm ss's' a");
	        //System.out.println("Timestamp:"+ df.format(pdu.getSmscTimeStamp().getTime()));
	       
	      // System.out.println("PDU:"+SmsDeliveryPdu);
		     
	        String decodedMessage = null;
	        int dataCodingScheme = pdu.getDataCodingScheme();
	        if (dataCodingScheme == PduUtils.DCS_ENCODING_7BIT) {
	            decodedMessage = PduUtils.decode7bitEncoding(null, bytes);
	            //.out.println("7bit");
	        } else if (dataCodingScheme == PduUtils.DCS_ENCODING_8BIT) {
	            decodedMessage = PduUtils.decode8bitEncoding(null, bytes);
	            //System.out.println("8bit");
	        } else if (dataCodingScheme == PduUtils.DCS_ENCODING_UCS2) {
	            decodedMessage = PduUtils.decodeUcs2Encoding(null, bytes);
	            //System.out.println("ucs2");
	        } else {
	            System.out.println("Unknown DataCodingScheme!");
	           
	        }
			return decodedMessage;
		}
	    String formatTimestamp(Calendar timestamp)
		{
			SimpleDateFormat sdf = new SimpleDateFormat();
			sdf.applyPattern("EEE dd-MMM-yyyy HH:mm:ss z");
			sdf.setTimeZone(timestamp.getTimeZone());
			return sdf.format(timestamp.getTime());
		}
	   public  String toHexString(byte[] arrby, int n) {
	        if (arrby == null || arrby.length < 1) {
	            throw new IllegalArgumentException("this byteArray must not be null or empty");
	        }
	        StringBuilder stringBuilder = new StringBuilder(n * 2);
	        for (int i = 0; i < n; ++i) {
	            if ((255 & arrby[i]) < 16) {
	                stringBuilder.append("0");
	            }
	            stringBuilder.append(Integer.toHexString((int)(255 & arrby[i])));
	            if (i == -1 + arrby.length) continue;
	            stringBuilder.append("");
	        }
	        return stringBuilder.toString().toUpperCase();
	    }
	    String CSMSconverter()
		{
			Date now = Calendar.getInstance().getTime();
			DateFormat df = new SimpleDateFormat("MMddmmss");
			String x=df.format(now);
			
	        String csms = intToHex(Math.round((Float.valueOf(Integer.valueOf(x.substring(4,6))*60+Integer.valueOf(x.substring(6,8)))*250)/3600));
	        //System.out.println(csms);
			return csms;
		}
	   private  String intToHex(int i) {
	        String hex = Integer.toHexString(i);
	        if (hex.length() % 2 != 0) hex = 0 + hex;
	        return hex.toUpperCase();
	    }
	   public  void UpsertSMS(JSONObject jObject) throws JSONException {
		   boolean found = false;
		   int i;int j=0;String UDH1="";String millis1="";String MpSeqNo="0";
		   Boolean isConcat=jObject.getBoolean("isConcat");
		   String data=jObject.getJSONArray("data").get(0).toString();		   
		   String CMGL=jObject.getJSONArray("CMGL").get(0).toString();
		   if(isConcat) {
			   MpSeqNo=jObject.getJSONArray("MpSeqNo").get(0).toString();
			   UDH1=jObject.getString("UDH");		            
	           millis1=jObject.getJSONArray("millis").get(0).toString();
	       }
		   for (i = 0; i < jsonArray.length(); i++) {
	            JSONObject obj =jsonArray.getJSONObject(i);
	            if(isConcat && obj.getBoolean("isConcat")) {
		            String UDH0=obj.getString("UDH");
		            String millis2=obj.getJSONArray("millis").get(0).toString();  
		            
		            JSONArray millis0=obj.getJSONArray("millis");
		            JSONArray CMGL0=obj.getJSONArray("CMGL");
		            JSONArray MpSeqNo0=obj.getJSONArray("MpSeqNo");
		            JSONArray data0=obj.getJSONArray("data");
		            if(UDH0.equals(UDH1) && Math.abs(Long.valueOf(millis2)-Long.valueOf(millis1))<60000*60) {	
		            	JSONArray jArr=upsertConcat(millis1,CMGL,MpSeqNo,data,millis0,CMGL0,MpSeqNo0,data0);
		                jObject.put("data", jArr.get(3));
		            	jObject.put("MpSeqNo", jArr.get(2));
		                jObject.put("CMGL", jArr.get(1));
		                jObject.put("millis", jArr.get(0));/**/
		            	found=true;j=i;
		            }		            
	            }            
	        }
		   if(found){
			   jsonArray.put(j,jObject);
			    found=false;
           }
		   else{
	        	jsonArray.put(jObject);
	        }	
		  
	    }
	 
	  public  JSONArray upsertConcat(String millis,String CMGL,String MpSeqNo,String data,JSONArray millis0,JSONArray CMGL0,JSONArray MpSeqNo0,JSONArray data0){
		JSONArray millis1=new JSONArray();
		JSONArray CMGL1=new JSONArray();
		JSONArray MpSeqNo1=new JSONArray();
		JSONArray data1=new JSONArray();
		JSONArray ALL=new JSONArray();
		int count=0;
		int xl=MpSeqNo0.length();
		Boolean flag=true;
		if(MpSeqNo0.isEmpty()) {
			millis1.put(millis);
			CMGL1.put(CMGL);
			MpSeqNo1.put(MpSeqNo);
			data1.put(data);
		}else {
			if(xl>0) {
				for (int i=0;i < xl;i++) {
			        if (Integer.valueOf(MpSeqNo) > MpSeqNo0.getInt(i)) {
			        	count++;		        	
			        }
			        if (Integer.valueOf(MpSeqNo) == MpSeqNo0.getInt(i)) {
			        	flag=false;
			        	//System.out.println("test");	        	
			        }
			    }
			}
			for (int i=0;i < xl;i++) {
				if(flag) {
					if(count==i && xl!=1) {
						millis1.put(millis);
						CMGL1.put(CMGL);
						MpSeqNo1.put(MpSeqNo);
						data1.put(data);
					}
					if(count==0 && xl==1) {
						millis1.put(millis);
						CMGL1.put(CMGL);
						MpSeqNo1.put(MpSeqNo);
						data1.put(data);
					}
				}else {
					if(count==i && xl!=1) {						
						CMGL1.put(CMGL);
					}
				}
						millis1.put(millis0.get(i));
						CMGL1.put(CMGL0.get(i));
						MpSeqNo1.put(MpSeqNo0.get(i));
						data1.put(data0.get(i));
				
		    }
			
			if(count==xl) {
				if(flag) {
					millis1.put(millis);
					CMGL1.put(CMGL);
					MpSeqNo1.put(MpSeqNo);
					data1.put(data);
				}else {
					CMGL1.put(CMGL);
				}
			}
		}
		
		ALL.put(millis1);
		ALL.put(CMGL1);
		ALL.put(MpSeqNo1);
		ALL.put(data1);
		//System.out.println(ALL);
		return ALL;
	}
	public  void printSMS(JSONArray arr) throws IllegalAccessException {
		   for (int i = 0; i < arr.length(); i++) {
			   JSONObject obj =arr.getJSONObject(i);
			   Boolean complete=false;
			   if(obj.getInt("MpMaxNo")==obj.getJSONArray("MpSeqNo").length()) {complete=true;}
			   System.out.println(obj.getInt("MpMaxNo")+":"+complete);
			   System.out.println("+"+obj.getString("sender"));//.substring(2,12)
			   System.out.println("CMGL:"+obj.getJSONArray("CMGL"));
			   System.out.println(ArrToString(obj.getJSONArray("data")));
		   }		  
      }
	  public  List<String> CMGList(String str){
		  return Arrays.asList(str.split(","));
	  }
	   public  ArrayList<String> jItems(JSONObject jsob) throws IllegalAccessException {
			ArrayList<String> arr=new ArrayList<String>();
			for (Iterator<String> key = jsob.keys(); key.hasNext();) {
	            String item=key.next();
	            arr.add(item);
	            
	        }
	        return arr;
	    }
	   public  String JArrayString="[{\"CMGL\":\"4,5,6\",\"isConcat\":true,\"MpSeqNo\":\"123\",\"data\":\"abc\",\"sender\":\"639778572402\",\"UDH\":\"05000332\",\"MpMaxNo\":3,\"millis\":\"1671523439832,1671523440833,1671523442833\"}]";
	   public  JSONArray xcv=new JSONArray(JArrayString);
	   public  String readString="AT+CMGL=4\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 1,1,\"\",30\r\n"
	   		+ "0791361907002059040C913679874023540000321051911593230CC8329BFD065DDF72363904\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 2,1,\"\",105\r\n"
	   		+ "0791361907002059600C91361925367429000032106161832323620500039502025C361B0E77B3E136AF984CE69ACD6636986CF35241EB6C36E85DA783DA657A590E32BFE5A071989D16CBC3F4F4DB0D0ABBC9207979FC76BBCB637AFAED06BDCD2077F90E4ACFE7F53219D42ED3CB7210\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 3,1,\"\",159\r\n"
	   		+ "\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 4,1,\"\",159\r\n"
	   		+ "\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 5,1,\"\",159\r\n"
	   		+ "\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 6,1,\"\",60\r\n"
	   		+ "0791361908007403440C913609571200230000321071115573232E0500030F0202A05526B348043DAB54D0F28A04358BD4A214F4025D9354244898643241552734982402\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 7,1,\"\",159\r\n"
	   		+ "\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 8,1,\"\",152\r\n"
	   		+ "\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 9,1,\"\",152\r\n"
	   		+ "\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 10,1,\"\",152\r\n"
	   		+ "\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 11,1,\"\",34\r\n"
	   		+ "0791361907002059440C9136798740235400003210519144332311050003B90404E8207658FC96D7DB2E\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 12,1,\"\",75\r\n"
	   		+ "0791361907002059640C913619253674290000321071614355233F050003A302026031D9E61593C95CB3592C269B6D5E2A789DCD7ED7E9A0BABB3E2F9341F037BB0CA297DD6410FD0D1ABFD9EC307C5E268300\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 13,1,\"\",75\r\n"
	   		+ "0791361907002059640C913619253674290000321071615374233F050003A302026031D9E61593C95CB3592C269B6D5E2A789DCD7ED7E9A0BABB3E2F9341F037BB0CA297DD6410FD0D1ABFD9EC307C5E268300\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 14,1,\"\",159\r\n"
	   		+ "\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 15,1,\"\",35\r\n"
	   		+ "0791361907002059600C91361925367429000032106161647423120500039B020264AED96C6683C9362F15\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 16,1,\"\",159\r\n"
	   		+ "\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 17,1,\"\",68\r\n"
	   		+ "0791361907002059640C9136192536742900003210716163752338050003A5020262B1CD4B459587DD7373590E0ABBC9A031BB3EA797E569F719F43683DA657A590EA2BF41EEF21D047FB3CB\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 18,1,\"\",68\r\n"
	   		+ "0791361907002059040C9136192536742900003210713230652337ECF7384C4FBFDD9B570CE6B2D97035192EC78AC964AED96C96ABDD649B976C068BC56EB2196C56A36D5EC77ABA2D0EB301\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 19,1,\"\",68\r\n"
	   		+ "0791361908007146440C913639602318760000321071319275233705000330020240737AB9CC06C1D36E50DA3DAFB3C3F4B7DC059297E1ECF0B80C2287DBE1731964AECFCBA0582DDC86BB00\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 20,1,\"\",68\r\n"
	   		+ "0791361907002059040C9136192536742900003210713240252337ECF7384C4FBFDD9B570CE6B2D97035192EC78AC964AED96C96ABDD649B976C068BC56EB2196C56A36D5EC77ABA2D0EB301\r\n"
	   		+ "\r\n"
	   		+ "OK";
	   public  String readString1=""
	   		+ "AT+CMGL=4\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 4,1,\"\",159\r\n"
	   		+ "\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 11,1,\"\",159\r\n"
	   		+ "\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 12,1,\"\",159\r\n"
	   		+ "\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 13,1,\"\",147\r\n"
	   		+ "\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 5,1,\"\",159\r\n"
	   		+ "\r\n"
	   		+ "\r\n"
	   		+ "+CMGL: 6,1,\"\",148\r\n"
	   		+ "\r\n"
	   		+ "\r\n"
	   		+ "OK\r\n";
	   public  String smstest="omms\\09302395699230220102647230220103000\\*MERLINDA PATNONGON\\2420430400506007148090100200300400\\low\\*METER REPLACEMENT - STUCK UP METER\\*50900343  TG0372 SO. FLORES\\*09661642833\\*\\10.6772626\\122.4001828\\*";
	   public  String dataPdu="EF767BBE79C172B71BAE7693D16032992C2693DD60389A8C0693C96232D90D86A3C960B0CD4B350FB7E1ECF2E6158BC960B1198C0683D5603618EC068BE16039580C0693C16033188C06836D5EE8F419BD79A9362FD5E6A5DABC549B570CE6C2D572325BAE76DBBC6232996B66CBD17239D9E6A502";
	   public  String smsPdu="";
	   public  String smsPdu2="0791361907002059400C91367987754220000022213241258223A00500033A0301";";
	   public  String sentData="AT+CMGF=0\r\n"
	   		+ "\r\n"
	   		+ "OK\r\n"
	   		+ "AT+CMGS=154\r\n"
	   		+ "\r\n"
	   		+ "> \r\n"
	   		+ "> \r\n"
	   		+ "+CMGS: 241\r\n"
	   		+ "\r\n"
	   		+ "OK\r\n"
	   		+ "AT+CMGS=154\r\n"
	   		+ "\r\n"
	   		+ "> \r\n"
	   		+ "> \r\n"
	   		+ "+CMGS: 242\r\n"
	   		+ "\r\n"
	   		+ "OK\r\n"
	   		+ "AT+CMGS=143\r\n"
	   		+ "\r\n"
	   		+ "> \r\n"
	   		+ "> \r\n"
	   		+ "+CMGS: 243\r\n"
	   		+ "\r\n"
	   		+ "OK";	
	   
	   
	   
}
